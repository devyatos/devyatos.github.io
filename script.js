var about=
	["Государство", "Столица", "Континент", "Часть света",  "Разница во времени",  
	...
	outCountries: function (){
		...
		function outArrayData(array){
			...
			if (index == 0){
				var obj ="\
				<tr class=GObj>\
				<td class=GObj id=left_item>Географические объекты:</td>\
				<td colspan=2>&#"+symbol+array+"</td>\
				</tr>\
				";
			} else {
				var obj ="\
		...
		function outArrayData_sec(array){
			symbol += 1;
			if (index == 0){
				var obj ="\
				<tr class=GObj>\
				<td class=GObj id=left_item>Литературные произведения:</td>\
				<td colspan=2>&#"+symbol+array[0]+" \""+array[1]+"\" "+" (" +array[2]+");" +"</td>\
				</tr>\
				";
			} else {
				var obj ="\
		...
		function Events(keys,county){
			var len = keys.length;
			for (var i = 0; i < len; i++){
				var obj = "<td class=items><span class=year>"+keys[i]+"</span> - "+county[keys[i]]+"</td>";
				tab+=obj;
			}
		}
	
		function makeTableFromCountry (county){
			var gObj = county[6].map(outArrayData);
			index = 0;
			symbol = 10101;
			var lObj = county[7].map(outArrayData_sec);
			index = 0;
			symbol = 10101;
			var eObj = Object.keys(county[8]);
			tab="<table>";
			tab+= "\
			<tr>\
			<td colspan=3><h1>"+county[0]+" - "+county[1]+"</h1></td>\
			</tr>\
			<tr>\
			<td id=left><span>Континент</span></td><td>"+county[2]+"</td>\
			</tr>\
			<tr>\
			<td id=left><span>Часть света</span></td><td>"+county[3]+"</td>\
			</tr>\
			<tr>\
			<td id=left><span>Разница во времени</span></td><td>"+county[4]+"</td>\
			</tr>\
			<tr>\
			<td id=left><span>Денежная единица</span></td><td>"+county[5]+"</td>\
			</tr>\
			<tr><td></td><td></td></tr>\
			<tr><td></td><td></td></tr>\
			";
			gObj.map(function(item){tab+=item;});
			tab+="<tr><td></td><td></td></tr><tr><td></td><td></td></tr>";
			lObj.map(function(item){tab+=item;});
			tab+="<tr><td></td><td></td></tr><tr><td></td><td></td></tr>";
			tab+= "\
			<tr class=GObj>\
			<td colspan=3 id=center><span class=history>Исторические события</span></td>\
			</tr>\
			</table>\
			<table class=Dates>\
			<tr>\
			";
			Events(eObj, county[8]);
			tab+="</tr></table>";
			return tab;
		}
		var new_tab = this.Countries.map(makeTableFromCountry);
		var doc;
		var content = "";
		new_tab.forEach(
			function (info){
				content += info+"<br><br><br>";
			}
		);
		document.getElementById("tablet").innerHTML = content;
		return doc;
	}
};
...
function Obj2(){
	...
	this.changeCountries = function (data) {
		...
		const cond = (currentValue) => currentValue[1].length >= data;
		var check;
		for (var i = 0; i < len; i++){
			check = false;
			check = this.Countries[i][7].every(cond);
			var name = this.Countries[i][0];
			if (check == true){
				this.Countries[i][0] = name.toUpperCase();
				console.log(countries[i][0]);
			} else {
				this.Countries[i][0] = name[0]+name.slice(1).toLowerCase();
			}
		}
	};
};
...
function Timer(title, date){
	...
	this.setTimer = function(){
		let timerId = setInterval(function(){
			...
			let two = new Date();
			var remaining = one - two; 
			var days;
			var hours;
			var minutes;
			var seconds;
			seconds = Math.round((remaining/1000)%60);
			minutes = Math.round((remaining/1000)/60)%60;    
			hours = Math.round(((remaining/1000)/60)/60)%24;    
			...
			";
			document.getElementById("timer"+timerId).innerHTML = time;
		}, 1000); 
	};
}

timerEOS = new Timer("До конца 2020 учебного года", "2020-06-30");
timerEOS.setTimer();

function btnClick(){
	var form = document.forma.chars.value;
	obj2.changeCountries(Number(form));
